...
it("renders a shared static navigation frame around route content", () => {
  ...
  expect(markup.match(/data-slot="navigation-menu-item"/g) ?? []).toHaveLength(3);
  expect(markup.match(/data-slot="navigation-menu-link"/g) ?? []).toHaveLength(3);
  expect(markup).toMatch(/<a[^>]*href="\/#capabilities"[^>]*>How it works<\/a>/);
  expect(markup).toMatch(/<a[^>]*href="\/#faq"[^>]*>FAQ<\/a>/);
  expect(markup).toMatch(/<a[^>]*href="\/blog"[^>]*>Blog<\/a>/);
...
it("keeps every Chinese public homepage link on /zh", () => {
  ...
  expect(markup).toContain('data-slot="navigation-menu"');
  expect(markup.match(/data-slot="navigation-menu-link"/g) ?? []).toHaveLength(3);
  expect(markup).toMatch(/<a[^>]*href="\/zh#capabilities"[^>]*>使用方式<\/a>/);
  expect(markup).toMatch(/<a[^>]*href="\/zh#faq"[^>]*>常见问题<\/a>/);
  expect(markup).toMatch(/<a[^>]*href="\/zh\/blog"[^>]*>博客<\/a>/);
  expect(markup).toContain("data-homepage-language-switcher");
  expect(markup).toContain('href="/"');

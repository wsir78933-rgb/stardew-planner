...
角色：Agent C-zh（只写作）。不执行 D 的三门，不签发通过，不用七罪写 Title / H1 / Description，不改 `src/` / `public/`。用户锁定 slug：`fall-crops-stardew`。locale=`zh-CN`。输入：`A-zh-research.md`（核验日 2026-09-14）、`B-zh-layout.md`。读者正文从下一节起，到「编辑附录」之前。

秋 1 先用镰刀清掉非当季枯株；夏 28 还在地里的玉米不枯。皮埃尔秋 1 就卖蔓越莓、南瓜、葡萄种子，秋 16 [星露谷展览会](https://zh.stardewvalleywiki.com/星露谷展览会)不是种子摊。只种浇得完的格：日均高不等于今年买得到、浇得完，也不要把一季总净利和日均合成另一个「最赚」。
...
C-zh 候选稿。未执行 D 三门，未签发通过，未写 Title / H1 / Description。未改 `src/` / `public/`。未把英文稿译成中文成稿。
...
- id: `wiki-amaranth`
  - label: 星露谷物语官方中文维基：苋菜（表内数字来自已打开秋季表；作物名链对应条目）
  - url: https://zh.stardewvalleywiki.com/苋菜
  - appliesTo:
    - quote: "皮埃尔 70 金，Joja 87 金"
      occurrence: 1

未列入公开引用、数字不进事实：灰机wiki「星露谷集会」、Bilibili 口播、Reddit 评论、3DM 南瓜 330 / 120 株七八万、贴吧蔓越莓 130、Farming Game Hub 29/23 G/Day。

### 图位说明

封面不能代替正文图。三张正文图均为可控绘图示意图，禁止冒充游戏截图。图中文字只许 A 已核验作物名与标明推算的日期。不要画斧头敲击次数，不要画春季花椰菜/草莓或夏季蓝莓/甜瓜。

1. `/blog/illustrations/fall-crop-occupancy-calendar-zh.webp`  
   落在「同一套日均口径下的秋季作物比较」表后。读者要看出：蔓越莓秋 1 种下占整季，收日 8/13/18/23/28 才可能满 5 次；南瓜单次 13 天，两茬推算秋 1→秋 14 收、当天再种→秋 27 收；只赶一茬最晚秋 15；漏浇后移。图注已写「推算，不是维基字段」。

2. `/blog/illustrations/fall-grape-walk-zh.webp`  
   落在「鲜卖、架子过道：葡萄」。精确格子图。读者要看出：两行架子夹一条空路才能走到每一株；任意生长阶段不能从植株上走过去。

3. `/blog/illustrations/fall-pumpkin-3x3-zh.webp`  
   落在「巨大南瓜按 3×3 排」。精确格子图。读者要看出：九格同种南瓜才有资格；洒水器占一格则无资格；左上角须成熟且浇水；Giant Pumpkin 外观 ≠ 九格资格。不要在九格里画蔓越莓或葡萄。

规划器截图不要求。若用规划器导出当底图，必须人工标注，且中文搜索框不要画成能搜到「南瓜」。

### 因未取得而拒绝写的点

- 巨大作物斧头敲几下：中文农作物页未写；英文 Crops 未开；本地 `Health: 3` 不代替维基句。
- 金库四档合计：本次沙漠页未给，不从赚钱文或英文夏文抄 42500。
- PAA：未取得，FAQ 只用 B 规定的 6 题。
- Google「星露谷 秋天种什么」「秋季最赚钱」「南瓜」「蔓越莓」排名：未取得，不编排名。
- Bilibili 口播数字、233乐园正文主体：未取得，不当作物表。
- 精确中国 Google 定位：页脚「无法确定位置」，正文不声称中国区 Google。
- 游戏补丁号 1.6.x 精确号：秋季页只显示 2026-01-17 编辑，不写补丁号冒充页脚（西蓝花「1.6 加入」、上古种子「1.6.3」按专页历史写）。
- 农作物页向日葵节「/天」单元格抽取丢失：日均只采用秋季表 −15.00；Joja −5.625 标为分析。
- 3DM 南瓜售价 330、120 株七八万、秋季总资产；贴吧蔓越莓 130；Hub 29/23 G/Day：已打开或摘要冲突，禁止当事实。
- 上古水果普通售价：A 室外事实表未给单价，表中未单列该行；只写专页/农作物页已给的生长、货车价冲突与冬 1 成熟。
- 洋蓟 Joja 价：农作物页未列，不编。
- 玉米收满 4 次的精确最晚播种日：A 只写「须更早」，不自行重算成秋 2。
- 西蓝花收满 5 次的精确最晚播种日：A 推算列只有「首次秋 20」，不自行补 5 次日期。

### 变更对照（对 D-zh FAIL 2 条）

C-zh 只改读者正文上述原句与可选口吻。不自检三门，不签发通过。请 D 另检本版本。

| D 条 | 改法 |
|------|------|
| RT-1 | 删「本文不用 12.96」。改为：秋季表西蓝花印 ≈12.96，分母 27 与农作物公式 24 天不一致；比日均时用表上的 ≈14.58，不用秋季表 12.96，也不在 12.96 和 14.58 之间折中。 |
| RT-2 | 删「可以进表，门槛必须写成」。改为：宝石甜莓 83.33 可以拿来比，但不是秋 1 默认货架；今年没这粒种子就整行跳过。 |
| 可选 向日葵 | 删「必须留在表里当反例，不是排版漏了」。改为：日均 −15.00，皮埃尔种子 200 金、花 80 金，现卖日均是负的，别当这季主力。未删 −15.00。 |
| 可选 placeholder | 「搜索框 placeholder 是」改为「搜索框里写着」。未删中文名 0 条 / 搜英文 Pumpkin。 |
| 可选 图后复述 | 删葡萄图后「空路不是浪费」复述段、南瓜图后资格复述段。图前讲解与图 alt 保留。目录外观不等于九格资格仍在图前第 132 行。 |

PublicReference：`wiki-fall`、`wiki-sweet-gem` 的 quote 已换成新读者句。未改 src/ 或 public/。未补斧头敲几下、金库四档合计、3DM/贴吧/Hub 数字。

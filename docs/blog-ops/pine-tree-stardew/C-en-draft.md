# C-en draft: pine-tree-stardew

Role: Agent C-en (writing only). Candidate en-US reader body. Not D’s three gates, not E’s independent appraisal, not F’s lock, and not a pass verdict. Not Title / H1 / Description, page source, public assets, or tests. Keyword: `pine tree stardew`. Inputs: `A-en-research.md` (2026-09-22), `B-en-layout.md`, and `A-media-plan.md`.
...
## Editor notes (not reader body)

- A follow-up is still required before any later locked version can claim one universal unfertilized Pine maturity number: the Pine Tree, Pine Cone, and Trees pages conflict on median/percentile wording.
- Do not add a cross-tree “most profitable” answer without a new same-condition comparison covering Tapper type, profession, time horizon, availability, and product basket.
- Do not add a numeric Pine physical footprint or a Pine Green Rain conversion claim; those facts were not verified in A-en research.
- The figure paths, final image bytes, alt text, and captions remain media/assembly inputs; this draft specifies teaching slots and does not claim that assets exist.
